...
When multiple slices in the same module reuse the same application service, that service MAY live in `Features/{Module}/Shared/`. A slice SHALL NOT call another slice’s endpoint/handler as its primary collaboration path.
